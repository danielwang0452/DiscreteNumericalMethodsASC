...
if __name__ == '__main__':
    ...
    hyperparameters = {  # lr, temp according to table 8 for VAE with 8x4 latents
        ...
        ('gumbel', 64, 8): [0.0003, 0.5, 'Adam'],
        ('rao_gumbel', 64, 8): [0.0005, 0.5, 'Adam'],
        ('gst-1.0', 64, 8): [0.0005, 0.7, 'Adam'],
        ('st', 64, 8): [0.001, 1.3, 'Adam'],
        ('reinmax', 64, 8): [0.0005, 1.3, 'Adam'],
        ('reinmax_v2', 64, 8): [0.0005, 1.0, 'Adam'],
        ('reinmax_v3', 64, 8): [0.0005, 1.0, 'Adam'],
    }
    latent_dim, categorical_dim = 10, 30
    methods = ['reinmax', 'gumbel', 'st', 'rao_gumbel', 'reinmax_v3']#gst-1.0', 'rao_gumbel']
    ...
    for m, method in enumerate(methods):
        ...
        # load pretrained VAE
        checkpoint = torch.load(f'/Users/danielwang/PycharmProjects/ReinMax_ASC/model_checkpoints/vae_{latent_dim}x{categorical_dim}_epoch_50.pth', map_location=device)
    ...
    # plot jacobians
    def visualise_jacobians(jacobian_dict, num_samples=2, save_path=f"saved_figs/jacobians.png"):
        ...
        for i, method in enumerate(methods):
            ...
            for j, idx in enumerate(idxs):
                ...
                ax = axes[i, j]
                im = ax.matshow(mat, cmap="seismic")  # diverging colormap
                rank = np.linalg.matrix_rank(mat)

                ax.set_title(f"{method} | rank={rank}")
                ax.axis("off")

                # add individual colorbar for clarity
    ...
    def visualise_jacobian_eigenvalues(jacobian_dict, num_samples=3, save_path=f"saved_figs/jacobian_eigenvalues.png"):
        """
        Visualize eigenvalue distributions of Jacobians from a dict of {method_name: (BL, C, C) tensor}.

        Shows histograms of eigenvalues for `num_samples` Jacobian matrices per method.
        """
        methods = list(jacobian_dict.keys())
        n_methods = len(methods)
        ...
        for i, method in enumerate(methods):
            ...
            for j, idx in enumerate(idxs):
                ...
                # compute eigenvalues (real part for possible complex eigenvalues)
                eigvals = np.linalg.eigvals(mat).real

                ax = axes[i, j]
                ax.hist(eigvals, bins=20, color="skyblue", edgecolor="black", alpha=0.8)
                ax.set_title(f"{method} | rank={np.linalg.matrix_rank(mat)}")
        ...
        plt.savefig(save_path, dpi=200)
        plt.close(fig)
        print(f"Saved eigenvalue visualization to {save_path}")


    def visualise_jacobians_and_eigenvalues(jacobian_dict, num_samples=5, save_path=f"saved_figs/jacobian_and_eigenvalues.png"):
        """
