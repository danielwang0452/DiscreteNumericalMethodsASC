...
def run(
    ...
    fname = f"./models/model_seed{seed}_{method}_cat{categorical_dim}_lat{latent_dim}_opt{optimizer_type}_lr{learning_rate}_temp{temperature}_epoch{epochs}.pt"
    if os.path.exists(fname):
        print(f"Model {fname} already exists. Skipping training.")
        return
    # set random seed
    random.seed(seed)
    np.random.seed(seed)
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        if epoch % save_interval == 0 or epoch == epochs or epoch == 1:
            torch.save(
                model.state_dict(),
                f"./models/model_seed{seed}_{method}_cat{categorical_dim}_lat{latent_dim}_opt{optimizer_type}_lr{learning_rate}_temp{temperature}_epoch{epoch}.pt",
            )


def check_sample_variance(
    epoch,
    seed,
    method,
    categorical_dim,
    ...
    bstd, norm = None, None
    tries = 10
    while bstd is None or torch.isnan(bstd).any():
        bstd, norm = model.get_sample_variance(
            data[:gradient_estimate_sample, :], 512
        )
        if tries == 0:
            print("Failed to compute variance without NaNs after multiple attempts.")
            break
        tries -= 1
        print("NaN encountered in variance calculation, retrying...")
    metrics = []
    ...
    # save metrics
    np.savetxt(
        f"./results/variance_{trained_method}_epoch{epoch}_seed{seed}_method_{method}_cat{categorical_dim}_lat{latent_dim}_opt{optimizer_type}_lr{learning_rate}_temp{temperature}.txt",
        np.array(metrics),
        delimiter=",",
    )
